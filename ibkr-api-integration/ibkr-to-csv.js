 ...
 *   position.avgCost      → Cost Price (stocks: per-share)
 *   position.avgPrice     → Cost Price (options: per-share; avgCost = avgPrice * 100)
...
class CSVGenerator {
  ...
  generate({ summary, ledger, positions, trades = [], priorNav = null, prices = {} }) {
    ...
    // ── Net Asset Value ────────────────────────────────────────
    const nav = this._buildNAV(summary, ledger, positions, priorNav, trades);
  ...
  _splitPositions(positions) {
    ...
    for (const p of positions) {
      ...
      else if (p.assetClass === 'OPT') options.push(p);
    }
  ...
  _normalizeOptionSymbol(p) {
    // Target format: "MRNA 06MAR26 45.50 P"
    // API contractDesc: "MRNA   MAR2026 45.50 P [MRNA  260306P00045500 100]"
    // strike/expiry/putOrCall fields are null — parse everything from contractDesc.
    //
    // Bracket portion has the canonical OCC-style data:
    //   MRNA  260306P00045500 100
    //         YYMMDD^         ← expiry
    //               ^         ← P or C
    //                ^^^^^^^^ ← strike * 1000 (8 digits, zero-padded)
    const desc = p.contractDesc || '';
    const bracketMatch = desc.match(/\[(\S+)\s+(\d{6})([PC])(\d{8})\s+\d+\]/);
    if (bracketMatch) {
      const ticker = bracketMatch[1];
      const yymmdd = bracketMatch[2]; // e.g. "260306"
      const pc = bracketMatch[3];      // "P" or "C"
      const strikeRaw = bracketMatch[4]; // e.g. "00045500"
      const yy = yymmdd.substring(0, 2);
      const mm = parseInt(yymmdd.substring(2, 4));
      const dd = yymmdd.substring(4, 6);
      const months = ['JAN','FEB','MAR','APR','MAY','JUN','JUL','AUG','SEP','OCT','NOV','DEC'];
      const strike = parseFloat(strikeRaw) / 1000;
      const strikeFmt = strike % 1 === 0 ? String(strike) : strike.toFixed(2).replace(/0+$/, '');
      return `${ticker} ${dd}${months[mm - 1]}${yy} ${strikeFmt} ${pc}`;
    }
    // Fallback: if contractDesc already has dashboard format (DDMMMYY)
    if (/\d{2}[A-Z]{3}\d{2}/.test(desc)) {
      return desc.trim();
    }
    return desc || 'UNKNOWN';
  }
  ...
  _buildNAV(summary, ledger, positions, priorNav, trades = []) {
    ...
    // Realized P/L: summary.realizedpnl is often undefined.
    // Fallback 1: sum realizedPnl from trades.
    // Fallback 2: ledger's realizedpnl field.
    let realizedPL = s.realizedpnl?.amount || 0;
    if (!realizedPL && Array.isArray(trades) && trades.length > 0) {
      realizedPL = trades.reduce((sum, t) => sum + (parseFloat(t.realizedPnl) || 0), 0);
    }
    if (!realizedPL) {
      realizedPL = l.realizedpnl || 0;
    }

    // Unrealized Change: sum unrealizedPnl from all positions
    const unrealizedChange = positions.reduce((sum, p) => sum + (p.unrealizedPnl || 0), 0);
    ...
    return {
      ...
      unrealizedChange: this._round(unrealizedChange),
