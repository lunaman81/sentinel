...
class IBKRApi {
  constructor(config = {}) {
    ...
    this.apiPrefix = config.apiPrefix || '/v1/api';
  ...
  _request(method, path, body = null) {
    return new Promise((resolve, reject) => {
      ...
      const opts = {
        ...
        headers: { 'Content-Type': 'application/json', 'User-Agent': 'ibkr-api-integration/1.0' },
        agent: this.agent,
      };
  ...
  async checkAuth() {
    const status = await this.get('/iserver/auth/status');
  ...
  async waitForAuth(maxWaitMs = 30_000) {
    ...
    throw new Error(`Authentication timeout. Complete login in browser at ${this.baseUrl}`);
  }
